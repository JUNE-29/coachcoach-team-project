-- 회원정보
DROP TABLE IF EXISTS `MY_SCHEMA`.`members` RESTRICT;

-- 코치
DROP TABLE IF EXISTS `MY_SCHEMA`.`coaches` RESTRICT;

-- 코칭프로그램
DROP TABLE IF EXISTS `MY_SCHEMA`.`coaching_programs` RESTRICT;

-- 운동태그
DROP TABLE IF EXISTS `MY_SCHEMA`.`workout_tags` RESTRICT;

-- 코칭방식
DROP TABLE IF EXISTS `MY_SCHEMA`.`coaching_systems` RESTRICT;

-- 코칭프로그램게시판
DROP TABLE IF EXISTS `MY_SCHEMA`.`coaching_program_boards` RESTRICT;

-- 회원코칭프로그램
DROP TABLE IF EXISTS `MY_SCHEMA`.`member_coaching_programs` RESTRICT;

-- toDoList
DROP TABLE IF EXISTS `MY_SCHEMA`.`to_do_list` RESTRICT;

-- 운동내역
DROP TABLE IF EXISTS `MY_SCHEMA`.`workout_list` RESTRICT;

-- 음식게시판
DROP TABLE IF EXISTS `MY_SCHEMA`.`food_boards` RESTRICT;

-- 음식게시판댓글
DROP TABLE IF EXISTS `MY_SCHEMA`.`food_board_comments` RESTRICT;

-- 체중
DROP TABLE IF EXISTS `MY_SCHEMA`.`weight` RESTRICT;

-- 회원코칭프로그램일정
DROP TABLE IF EXISTS `MY_SCHEMA`.`member_program_calendar` RESTRICT;

-- 첨부파일
DROP TABLE IF EXISTS `MY_SCHEMA`.`calendar_files` RESTRICT;

-- 코칭프로그램운동태그
DROP TABLE IF EXISTS `MY_SCHEMA`.`coaching_program_tags` RESTRICT;

-- 회원운동태그
DROP TABLE IF EXISTS `MY_SCHEMA`.`member_workout_tag` RESTRICT;

-- 코칭프로그램방식
DROP TABLE IF EXISTS `MY_SCHEMA`.`program_coaching_systems` RESTRICT;

-- 운동리스트태그
DROP TABLE IF EXISTS `MY_SCHEMA`.`workout_list_tag` RESTRICT;
...
CREATE TABLE `MY_SCHEMA`.`members` (
  ...
  `withdrawal_date`   DATETIME     NULL     DEFAULT now() COMMENT '탈퇴날짜', -- 탈퇴날짜
...
-- 코치
CREATE TABLE `MY_SCHEMA`.`coaches` (
  ...
  `area`              VARCHAR(255) NULL     COMMENT '수업가능지역', -- 수업가능지역
  `career`            TEXT         NULL     COMMENT '경력사항', -- 경력사항
  `certification`     TEXT         NULL     COMMENT '보유자격증', -- 보유자격증
  `introduce`         TEXT         NULL     COMMENT '강사소개', -- 강사소개
  ...
  `withdrawal`        INTEGER      NULL     COMMENT '탈퇴여부', -- 탈퇴여부
  `withdrawal_date`   DATETIME     NULL     COMMENT '탈퇴날짜', -- 탈퇴날짜
  `withdrawal_reason` TEXT         NULL     COMMENT '탈퇴이유' -- 탈퇴이유
)
COMMENT '코치';

-- 코치
ALTER TABLE `MY_SCHEMA`.`coaches`
...
-- 코칭프로그램
CREATE TABLE `MY_SCHEMA`.`coaching_programs` (
...
-- 코칭프로그램
ALTER TABLE `MY_SCHEMA`.`coaching_programs`
...
-- 운동태그
CREATE TABLE `MY_SCHEMA`.`workout_tags` (
...
-- 운동태그
ALTER TABLE `MY_SCHEMA`.`workout_tags`
...
-- 코칭방식
CREATE TABLE `MY_SCHEMA`.`coaching_systems` (
  `program_no` INTEGER NOT NULL COMMENT '프로그램번호' -- 프로그램번호
)
COMMENT '코칭방식';

-- 코칭방식
ALTER TABLE `MY_SCHEMA`.`coaching_systems`
...
-- 코칭프로그램게시판
CREATE TABLE `MY_SCHEMA`.`coaching_program_boards` (
...
-- 코칭프로그램게시판
ALTER TABLE `MY_SCHEMA`.`coaching_program_boards`
...
-- 회원코칭프로그램
CREATE TABLE `MY_SCHEMA`.`member_coaching_programs` (
...
-- 회원코칭프로그램
ALTER TABLE `MY_SCHEMA`.`member_coaching_programs`
...
-- toDoList
CREATE TABLE `MY_SCHEMA`.`to_do_list` (
...
-- toDoList
ALTER TABLE `MY_SCHEMA`.`to_do_list`
...
-- 운동내역
CREATE TABLE `MY_SCHEMA`.`workout_list` (
  `workout_list_no` INTEGER     NOT NULL COMMENT '운동내역번호', -- 운동내역번호
  `member_no`       INTEGER     NOT NULL COMMENT '일반회원번호', -- 일반회원번호
  `workout_date`    DATE        NOT NULL DEFAULT now() COMMENT '시행날짜', -- 시행날짜
  `walk_count`      INTEGER     NOT NULL COMMENT '걸음수', -- 걸음수
  `etc`             VARCHAR(50) NULL     COMMENT '비고' -- 비고
)
COMMENT '운동내역';

-- 운동내역
ALTER TABLE `MY_SCHEMA`.`workout_list`
...
-- 음식게시판
CREATE TABLE `MY_SCHEMA`.`food_boards` (
  `food_board_no` INTEGER      NOT NULL COMMENT '음식게시판번호', -- 음식게시판번호
  `member_no`     INTEGER      NOT NULL COMMENT '일반회원번호', -- 일반회원번호
  `photo`         VARCHAR(255) NULL     COMMENT '사진', -- 사진
  `content`       TEXT         NOT NULL COMMENT '내용', -- 내용
  `modified_date` DATETIME     NOT NULL DEFAULT now() COMMENT '수정일' -- 수정일
)
COMMENT '음식게시판';

-- 음식게시판
ALTER TABLE `MY_SCHEMA`.`food_boards`
...
-- 음식게시판댓글
CREATE TABLE `MY_SCHEMA`.`food_board_comments` (
...
-- 음식게시판댓글
ALTER TABLE `MY_SCHEMA`.`food_board_comments`
...
-- 회원코칭프로그램일정
CREATE TABLE `MY_SCHEMA`.`member_program_calendar` (
  `calendar_no`       INTEGER  NOT NULL COMMENT '회원코칭프로그램일정번호', -- 회원코칭프로그램일정번호
  `member_program_no` INTEGER  NOT NULL COMMENT '회원코칭프로그램번호', -- 회원코칭프로그램번호
  `plan`              TEXT     NOT NULL COMMENT '운동계획', -- 운동계획
  `workout_date`      DATETIME NOT NULL DEFAULT now() COMMENT '시행일' -- 시행일
)
COMMENT '회원코칭프로그램일정';

-- 회원코칭프로그램일정
ALTER TABLE `MY_SCHEMA`.`member_program_calendar`
...
-- 첨부파일
CREATE TABLE `MY_SCHEMA`.`calendar_files` (
...
-- 첨부파일
ALTER TABLE `MY_SCHEMA`.`calendar_files`
...
-- 코칭프로그램운동태그
CREATE TABLE `MY_SCHEMA`.`coaching_program_tags` (
  `tag_no`     INTEGER NOT NULL COMMENT '운동태그번호', -- 운동태그번호
  `program_no` INTEGER NOT NULL COMMENT '프로그램번호' -- 프로그램번호
)
COMMENT '코칭프로그램운동태그';
...
-- 코칭프로그램운동태그
ALTER TABLE `MY_SCHEMA`.`coaching_program_tags`
...
-- 회원운동태그
CREATE TABLE `MY_SCHEMA`.`member_workout_tag` (
  `workout_no` INTEGER     NOT NULL COMMENT '운동번호', -- 운동번호
  `name`       VARCHAR(30) NOT NULL COMMENT '이름', -- 이름
  `unit`       INTEGER     NOT NULL COMMENT '운동단위' -- 운동단위
)
COMMENT '회원운동태그';

-- 회원운동태그
ALTER TABLE `MY_SCHEMA`.`member_workout_tag`
  ADD CONSTRAINT `PK_member_workout_tag` -- 회원운동태그 기본키
    PRIMARY KEY (
      `workout_no` -- 운동번호
    );

-- 코칭프로그램방식
CREATE TABLE `MY_SCHEMA`.`program_coaching_systems` (
  `program_no` INTEGER NOT NULL COMMENT '프로그램번호' -- 프로그램번호
)
COMMENT '코칭프로그램방식';

-- 코칭프로그램방식
ALTER TABLE `MY_SCHEMA`.`program_coaching_systems`
...
-- 운동리스트태그
CREATE TABLE `MY_SCHEMA`.`workout_list_tag` (
  `workout_no`      INTEGER NOT NULL COMMENT '운동번호', -- 운동번호
  `workout_list_no` INTEGER NOT NULL COMMENT '운동내역번호' -- 운동내역번호
)
COMMENT '운동리스트태그';
...
-- 운동리스트태그
ALTER TABLE `MY_SCHEMA`.`workout_list_tag`
  ADD CONSTRAINT `PK_workout_list_tag` -- 운동리스트태그 기본키
    PRIMARY KEY (
      `workout_no`,      -- 운동번호
      `workout_list_no`  -- 운동내역번호
    );
...
ALTER TABLE `MY_SCHEMA`.`coaching_programs`
  ADD CONSTRAINT `FK_coaches_TO_coaching_programs` -- 코치 -> 코칭프로그램
    ...
    REFERENCES `MY_SCHEMA`.`coaches` ( -- 코치
      `coach_no` -- 코치번호
    );
...
ALTER TABLE `MY_SCHEMA`.`coaching_systems`
  ADD CONSTRAINT `FK_coaching_programs_TO_coaching_systems` -- 코칭프로그램 -> 코칭방식
    ...
    REFERENCES `MY_SCHEMA`.`coaching_programs` ( -- 코칭프로그램
      `program_no` -- 프로그램번호
    );
...
ALTER TABLE `MY_SCHEMA`.`member_coaching_programs`
  ADD CONSTRAINT `FK_members_TO_member_coaching_programs` -- 회원정보 -> 회원코칭프로그램
    ...
    REFERENCES `MY_SCHEMA`.`members` ( -- 회원정보
      `member_no` -- 일반회원번호
    );
...
ALTER TABLE `MY_SCHEMA`.`food_board_comments`
  ADD CONSTRAINT `FK_food_boards_TO_food_board_comments` -- 음식게시판 -> 음식게시판댓글
    ...
    REFERENCES `MY_SCHEMA`.`food_boards` ( -- 음식게시판
      `food_board_no` -- 음식게시판번호
    );
...
ALTER TABLE `MY_SCHEMA`.`member_program_calendar`
  ADD CONSTRAINT `FK_member_coaching_programs_TO_member_program_calendar` -- 회원코칭프로그램 -> 회원코칭프로그램일정
    ...
    REFERENCES `MY_SCHEMA`.`member_coaching_programs` ( -- 회원코칭프로그램
      `member_program_no` -- 회원코칭프로그램번호
    );
...
ALTER TABLE `MY_SCHEMA`.`calendar_files`
  ADD CONSTRAINT `FK_member_program_calendar_TO_calendar_files` -- 회원코칭프로그램일정 -> 첨부파일
    ...
    REFERENCES `MY_SCHEMA`.`member_program_calendar` ( -- 회원코칭프로그램일정
      `calendar_no` -- 회원코칭프로그램일정번호
    );
...
ALTER TABLE `MY_SCHEMA`.`coaching_program_tags`
  ADD CONSTRAINT `FK_workout_tags_TO_coaching_program_tags` -- 운동태그 -> 코칭프로그램운동태그
    ...
    REFERENCES `MY_SCHEMA`.`workout_tags` ( -- 운동태그
      `tag_no` -- 운동태그번호
    );
...
-- 운동리스트태그
ALTER TABLE `MY_SCHEMA`.`workout_list_tag`
  ADD CONSTRAINT `FK_member_workout_tag_TO_workout_list_tag` -- 회원운동태그 -> 운동리스트태그
    FOREIGN KEY (
      `workout_no` -- 운동번호
    )
    REFERENCES `MY_SCHEMA`.`member_workout_tag` ( -- 회원운동태그
      `workout_no` -- 운동번호
    );

-- 운동리스트태그
ALTER TABLE `MY_SCHEMA`.`workout_list_tag`
  ADD CONSTRAINT `FK_workout_list_TO_workout_list_tag` -- 운동내역 -> 운동리스트태그
    FOREIGN KEY (
      `workout_list_no` -- 운동내역번호
    )
    REFERENCES `MY_SCHEMA`.`workout_list` ( -- 운동내역
      `workout_list_no` -- 운동내역번호
    );