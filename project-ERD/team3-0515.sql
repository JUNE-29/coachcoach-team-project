...
-- 운동내역
DROP TABLE IF EXISTS member_workouts RESTRICT;
...
CREATE TABLE members (
  ...
  userkey           VARCHAR(50)  NULL     COMMENT 'userkey', -- userkey
  auth_status       INTEGER      NULL     COMMENT 'auth_status' -- auth_status
)
COMMENT '회원정보';
...
ALTER TABLE member_workouts
  ADD CONSTRAINT PK_member_workouts -- 운동내역 기본키
    PRIMARY KEY (
...
CREATE TABLE member_program_calendar (
  ...
  start_date        DATETIME NOT NULL DEFAULT now() COMMENT '플랜시작일', -- 플랜시작일
  end_date          DATETIME NOT NULL COMMENT '플랜종료일' -- 플랜종료일
)
COMMENT '회원코칭프로그램일정';
...
CREATE TABLE workout (
  workout_no INTEGER     NOT NULL COMMENT '운동번호', -- 운동번호
  name       VARCHAR(30) NOT NULL COMMENT '운동종목' -- 운동종목
)
COMMENT '운동';
...
-- 운동시간2
CREATE TABLE workout_unit2 (
  workout_unit_no INTEGER NOT NULL COMMENT '운동번호', -- 운동번호
  workout_no      INTEGER NOT NULL COMMENT '운동번호2', -- 운동번호2
  unit            INTEGER NOT NULL COMMENT '운동시간' -- 운동시간
)
COMMENT '운동시간2';

-- 운동시간2
ALTER TABLE workout_unit2
  ADD CONSTRAINT PK_workout_unit2 -- 운동시간2 기본키
    PRIMARY KEY (
      workout_unit_no -- 운동번호
    );

-- 운동시간
CREATE TABLE workout_unit3 (
  workout_unit_no INTEGER NOT NULL COMMENT '운동번호', -- 운동번호
  workout_no      INTEGER NOT NULL COMMENT '운동번호2', -- 운동번호2
  unit            INTEGER NOT NULL COMMENT '운동시간' -- 운동시간
)
...
-- 운동시간
ALTER TABLE workout_unit3
  ADD CONSTRAINT PK_workout_unit3 -- 운동시간 기본키
    PRIMARY KEY (
      workout_unit_no -- 운동번호
    );
...
-- 운동내역
ALTER TABLE member_workouts
  ADD CONSTRAINT FK_members_TO_member_workouts -- 회원정보 -> 운동내역
    FOREIGN KEY (
      member_no -- 일반회원번호
    )
    REFERENCES members ( -- 회원정보
...
-- 운동단위
ALTER TABLE workout_unit
  ADD CONSTRAINT FK_workout_TO_workout_unit -- 운동 -> 운동단위
    FOREIGN KEY (
      workout_no -- 운동번호
    )
...
-- 운동리스트태그
ALTER TABLE workout_list_tag
  ADD CONSTRAINT FK_workout_unit_TO_workout_list_tag -- 운동단위 -> 운동리스트태그
    FOREIGN KEY (
      workout_unit_no -- 운동단위번호
    )
    REFERENCES workout_unit ( -- 운동단위
...
-- 운동리스트태그
ALTER TABLE workout_list_tag
  ADD CONSTRAINT FK_member_workouts_TO_workout_list_tag -- 운동내역 -> 운동리스트태그
    FOREIGN KEY (
      workout_list_no -- 운동내역번호
    )
